...
import { createReview, getAuthToken } from "../../../../lib/rental-api";
...
export default function RatingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!rentalId) return alert("ID rental tidak valid.");

    const API_URL = process.env.NEXT_PUBLIC_API_URL;
    const token = getAuthToken();
    if (!API_URL) return alert("Konfigurasi API belum tersedia.");

    setIsSubmitting(true);
    try {
      const result = await createReview(API_URL, token, rentalId, { rating, comment });
      if (!result.response.ok) {
        const message = String((result.json?.error as string) || "Gagal kirim rating.");
        throw new Error(message);
      }
      alert("Terima kasih atas rating Anda!");
      router.push("/user/history");
    } catch (error: any) {
      alert(error?.message || "Gagal kirim rating.");
    } finally {
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-x-clip bg-slate-50 px-4 py-12 text-slate-900">
      ...
      <div className="relative w-full max-w-md space-y-8 rounded-[2.5rem] border border-white/50 bg-white/80 p-8 shadow-2xl backdrop-blur-xl sm:p-12">
        <div className="text-center">
          ...
          <p className="mt-1 text-[10px] font-black text-indigo-500 uppercase tracking-widest leading-none">RENTAL ID: #{rentalId}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-10 space-y-8">
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-2">Komentar (Opsional)</label>
            <textarea
              value={comment}
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => router.back()} className="flex-1 rounded-2xl border border-slate-200 py-4 text-xs font-black uppercase tracking-widest text-slate-500 transition hover:bg-slate-50">
              Batal
            </button>
            <button type="submit" disabled={isSubmitting || rating === 0} className="flex-[2] rounded-2xl bg-indigo-600 py-4 text-xs font-black uppercase tracking-widest text-white shadow-xl shadow-indigo-200 transition-all hover:bg-indigo-700 disabled:opacity-50">
