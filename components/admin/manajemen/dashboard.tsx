...
import { 
  fetchAdminRentals, // Diubah agar sinkron dengan Validasi
  fetchAdminReviews,
  formatIdr, 
  getAuthToken, 
  rentalStatusLabel 
...
export default function Dashboard({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: any }) {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Menggunakan fetchAdminRentals agar status 'active' terdeteksi secara real-time
        const [{ rows }, { rows: reviews }] = await Promise.all([
          fetchAdminRentals(API_URL, token),
  ...
  const stats = useMemo(() => {
    // Menghitung record yang sah (tidak termasuk pending/canceled)
    const validRows = rawData.filter((row) => 
      ["active", "completed", "approved"].includes(String(row?.rental_status).toLowerCase())
    );
    ...
    // FIX: Menghitung status 'active' agar sinkron dengan label "Sedang Digunakan"
    const activeRentals = rawData.filter((row) => 
      String(row?.rental_status).toLowerCase() === "active"
    ).length;

    const totalLateFees = rawData.reduce((acc, row) => 
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 w-full max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
              <StatCard label="Total Penjualan (tanpa denda)" value={formatIdr(stats.totalRevenue)} color="text-blue-600" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
              <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[2rem] border border-slate-100 shadow-xl">
                <h3 className="font-black text-lg text-slate-800 mb-8">Unit Terpopuler</h3>
                <div className="space-y-3">
                  {popularUnits.length > 0 ? popularUnits.map(([name, count], i) => (
                    <div key={i} className="p-5 bg-slate-50 rounded-2xl flex justify-between items-center group hover:bg-slate-900 transition-all">
                      <span className="font-bold group-hover:text-white">{name}</span>
                      <span className={`text-[10px] px-3 py-1 rounded-full font-black ${i === 0 ? 'bg-orange-100 text-orange-600' : 'bg-slate-200 text-slate-600'}`}>
                        {count}x DISEWA
                      </span>
                    </div>
                  )) : <p className="text-slate-400 text-center py-4 italic text-sm">Belum ada data.</p>}
                </div>
              </div>

              <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[2rem] border border-slate-100 shadow-xl">
                <h3 className="font-black text-lg text-slate-800 mb-8">Pesanan Terbaru</h3>
                <div className="space-y-4">
                   {latestOrders.map((order) => {
                     const badge = rentalStatusLabel(order.rental_status, order.late_fee);
                     return (
